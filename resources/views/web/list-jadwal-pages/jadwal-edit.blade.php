    ...
    <form action="{{route('jadwal.update',$jadwal->id)}}" method="POST">
        ...
        <div class="mb-2">
            ...
            <select id="dropdownSelect" name="id_kelas"
                ...
                <option value="{{(old('id_kelas') ?? $jadwal->id_kelas) ? '' : 'selected'}}">Pilih kelas</option>
                @foreach (App\Models\Jadwal::getKelasValues() as $id => $kelas)
                    <option value="{{ $id }}" {{ (old('id_kelas') ?? $jadwal->id_kelas) == $id ? 'selected' : '' }}>{{ $kelas }}</option>
                @endforeach
            </select>
        </div>
        @error('id_kelas')
        ...
        <div class="mb-2">
            ...
            <select id="dropdownSelect" name="hari"
                ...
                <option value="{{(old('hari') ?? $jadwal->hari) ? '':'selected'}}">Pilih hari</option>
        ...
        <div class="mb-2">
            ...
            <input type="time" id="mulai" name="mulai"
                ...
                placeholder="" value="{{old('mulai') ?? $jadwal->mulai}}">
        </div>
        @error('mulai')
        <div class="text-red-500 text-sm mb-2">
            {{$message}}
        ...
        <div class="mb-2">
            ...
            <input type="time" id="selesai" name="selesai"
                ...
                placeholder="" value="{{old('selesai') ?? $jadwal->selesai}}">
        </div>
        @error('selesai')
        <div class="text-red-500 text-sm mb-2">
            {{$message}}
        ...
        <div class="mb-2">
            ...
            <select id="dropdownSelect" name="ruangan"
                ...
                <option value="{{(old('ruangan') ?? $jadwal->ruangan) ? '':'selected'}}">Pilih ruangan</option>
        ...
        <div class="mb-2">
            ...
            <select id="dropdownSelect" name="jenis"
                ...
                <option value="{{ $jenis }}" {{(old('jenis')??$jadwal->jenis)==$jenis?'selected':''}}>{{ $jenis }}</option>
                @endforeach
            </select>
        </div>
        @error('jenis')
